# ============================================================
# fix-postgres-credentials-final.ps1
# Финальная версия — передача Python-кода через stdin
# ============================================================
...
function Invoke-DockerPythonStdin {
    param(
        [Parameter(Mandatory)]
    ...
    $ErrorActionPreference = "Continue"
    try {
        $output = $PythonCode | & docker @DockerArguments 2>&1
        $exitCode = $LASTEXITCODE
    } finally {
        $ErrorActionPreference = $oldPreference
    }
...
Write-Host "[7/17] Checking DATABASE_URL propagation (via stdin)..." -ForegroundColor Yellow
$checkCode = @'
import os
import sys
...
$checkResult = Invoke-DockerPythonStdin `
    -DockerArguments @(
        "compose",
        "run",
        "--rm",
...
$dbProbeResult = Invoke-DockerPythonStdin `
    -DockerArguments @(
        "exec",
        "-i",
        $backendId,
    ...
    -PythonCode $dbProbeCode

if ($dbProbeResult.ExitCode -ne 0) {
    Write-Host "  FAIL Backend cannot connect to PostgreSQL!" -ForegroundColor Red
    Write-Host "  Output: $($dbProbeResult.Output)" -ForegroundColor Yellow
    throw "Backend cannot connect to PostgreSQL"
}
